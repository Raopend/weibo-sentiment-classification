...
class LstmClassifier:
    def __init__(self):
        ...
        self.classifier.load_state_dict(
            torch.load(
                "backend/classifier/models/02_lstm_model.pth", map_location=self.device
            )
        )
        self.word2vec = gensim.models.Word2Vec.load(
            "backend/classifier/models/word2vec.model"
        )
    ...
    def predict(self, text: str) -> Tuple[Dict[str, float], str, float]:
        ...
        with torch.inference_mode():
            ...
            probabilities = torch.round(probabilities * 10000).squeeze() / 100
            sentiment = "positive" if probabilities.item() > 50 else "negative"
            confidence = probabilities.item()
        return (
            {"positive": confidence, "negative": 100 - confidence},
            sentiment,
            confidence,
        )
